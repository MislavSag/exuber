...
# Parameters
FREQ = "week" # can be week and month
...
# Daily
cols = c("symbol", "date", "exuber_1000_0_sadf_log", "exuber_1000_0_gsadf_log", "exuber_1000_0_bsadf_log")
...
#                  by = .(symbol, freq)]
exuberm = exuber[, lapply(.SD, function(x) mean(x) / sd(x)),
                 by = .(symbol, freq), .SD = -"date"]

# Checks
exuber[symbol == "aapl"]
plot(exuber[symbol == "aapl", exuber_1000_0_bsadf_log], type = "l")
...
dt_long = dt[
  # beta %between% c(0, 1) &
  close_raw %between% c(1, 20) &
    volume > 100000,
  # .SD[shift(close, 1) / shift(close, 4*12) > 0],
  # by = symbol
][var_ntile %in% c(1, 40), .(
  symbol,
  freq,
...
setorderv(dt_long, c("freq", "var"))
dt_long = dt_long[, head(.SD, 10), by = freq] # FOR TEST
...
if (FREQ == "week") {
  ...
  portfolio_xts = as.xts.data.table(portfolio[, .(month = zoo::as.Date.yearmon(freq), target)])
} else if (FREQ == "day") {
...
# dt_long = dt[
#   # beta %between% c(0, 1) &
#   close_raw %between% c(0, 1000) & volume > 100000 &
# ][var_ntile %in% 1, .(
#   symbol,
#   freq,
#   target,
...
#                                Sys.getenv("BLOB-KEY"))
if (FREQ == "week") {
  qc_data = dt_long[, .(symbol, date = freq)]
} else if (FREQ == "month") {
  qc_data = dt_long[, .(symbol, date = as.Date(zoo::as.yearmon(freq)))]
